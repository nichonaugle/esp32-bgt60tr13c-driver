...
import json
import os
# Updated scipy imports for the new logic
...
class SimpleCalibration:
    def __init__(self):
        ...
        self.motion_sensitivity = 3.0   # Energy threshold for a pixel
        
        # ⭐ NEW motion detection parameters
        self.cluster_dilation = 2       # How far to search to connect nearby pixels (1=none)
        self.motion_score_threshold = 50 # Threshold for the weighted motion score
    ...
    def detect_motion(self, range_doppler_db):
        """ ⭐ NEW: Motion detection with Dilation and Weighted Scoring. """
        if self.doppler_baseline is None:
            return 0.0, False

        # 1. Define ROI and ignore static zone
        roi = range_doppler_db[:, 10:50]
        center_doppler = roi.shape[0] // 2
        roi[center_doppler-2:center_doppler+2, :] = -100

        # 2. Thresholding
        energy_threshold = self.doppler_baseline + self.motion_sensitivity
        motion_mask = roi > energy_threshold
        
        # 3. Dilation (The "Closeness" setting)
        # Connects pixels that are close but not touching
        struct_size = self.cluster_dilation
        structure = np.ones((struct_size, struct_size))
        dilated_mask = binary_dilation(motion_mask, structure=structure)

        # 4. Find Clusters in the dilated mask
        labeled_mask, num_features = label(dilated_mask)
        if num_features == 0:
            return 0.0, False

        # 5. Calculate Weighted Score for each cluster
        # This gives more weight to tall, streaky clusters (real motion)
        scores = []
        cluster_slices = find_objects(labeled_mask)
        for i, sl in enumerate(cluster_slices):
            cluster_pixels = (labeled_mask[sl] == i + 1)
            size = np.sum(cluster_pixels)
            
            # Calculate doppler span (height of the cluster)
            doppler_span = sl[0].stop - sl[0].start
            
            # The weighted score!
            score = size * (doppler_span ** 2)
            scores.append(score)
        ...
        # 6. Make Decision based on the highest score
        motion_detected = max_score > self.motion_score_threshold
        
        return max_score, motion_detected
    ...
    def save_calibration(self, filename="simple_radar_calibration.json"):
        data = {
            ...
            'cluster_dilation': self.cluster_dilation,
    ...
    def load_calibration(self, filename="simple_radar_calibration.json"):
        ...
        try:
            ...
            self.cluster_dilation = data.get('cluster_dilation', 2)
            self.motion_score_threshold = data.get('motion_score_threshold', 50)
...
class CalibrationControl:
    def __init__(self):
        ...
        self.window.geometry("400x580") # Increased height for new controls
        self.window.attributes('-topmost', True)

        self.presence_var = tk.StringVar(value="NO")
        self.motion_var = tk.StringVar(value="NO")
        self.motion_level_var = tk.StringVar(value="0")
    ...
    def setup_gui(self):
        ...
        # --- Calibration & Status Frames (unchanged) ---
        ...
        ttk.Label(status_frame, text="Max Motion Score:").grid(row=2, column=0, sticky='w', padx=5, pady=2)
        ttk.Label(status_frame, textvariable=self.motion_level_var).grid(row=2, column=1, sticky='w', padx=5, pady=2)
        # --- End of unchanged frames ---
        ...
        # ⭐ NEW GUI CONTROLS
        ttk.Label(sens_frame, text="Cluster Dilation (pixels):").pack(anchor='w', padx=5, pady=(10, 0))
        self.dilation_var = tk.IntVar(value=calibration.cluster_dilation)
        dilation_spinbox = ttk.Spinbox(sens_frame, from_=1, to=10, increment=1, textvariable=self.dilation_var, command=self.update_sensitivities, width=8)
        dilation_spinbox.pack(anchor='w', padx=5, pady=2)
        self.dilation_var.trace_add('write', self.update_sensitivities)

        ttk.Label(sens_frame, text="Motion Score Threshold:").pack(anchor='w', padx=5, pady=(10, 0))
        self.score_thresh_var = tk.DoubleVar(value=calibration.motion_score_threshold)
        score_thresh_spinbox = ttk.Spinbox(sens_frame, from_=0, to=10000, increment=10, textvariable=self.score_thresh_var, command=self.update_sensitivities, width=8)
        score_thresh_spinbox.pack(anchor='w', padx=5, pady=2)
        self.score_thresh_var.trace_add('write', self.update_sensitivities)
    ...
    def update_sensitivities(self, *args):
        try:
            ...
            calibration.cluster_dilation = self.dilation_var.get()
            calibration.motion_score_threshold = self.score_thresh_var.get()
            self.update_info()
    ...
    def load_calibration(self):
        if calibration.load_calibration():
            ...
            self.dilation_var.set(calibration.cluster_dilation)
            self.score_thresh_var.set(calibration.motion_score_threshold)
            self.update_info()
    ...
    def update_info(self):
        ...
        if calibration.range_baseline is not None:
            ...
            info += f"• Cluster Dilation: {calibration.cluster_dilation} px\n"
            info += f"• Motion Score Threshold: {calibration.motion_score_threshold}\n\n"
            info += "Detection Logic:\n"
            info += "• Motion Score = Size * (Height^2)\n"
            info += "• Detect if any cluster's score > threshold"
        else:
            info += "✗ Not calibrated\n\n"
...
# Create and run the GUI, serial parsing, and plotting loop.
                    ...
                    for c in range(NUM_CHIRPS_PER_FRAME):
                        for s in range(NUM_SAMPLES_PER_CHIRP):
                            for r_ant in range(NUM_RX_ANTENNAS):
                                rx_frame_channels[r_ant][c, s] = flat_data_1d[idx]; idx += 1
...
print("Starting radar with advanced motion scoring...")
print("Use the Control Window to calibrate and adjust sensitivity")
try:
    while True:
        if not plt.fignum_exists(fig.number): print("Plot window closed. Exiting."); break
        selected_antenna_frame_data, parsed_frame_num, should_exit = parse_frame_data_from_serial()
        if should_exit: break
        if selected_antenna_frame_data is None: plt.pause(0.05); continue
        ...
        raw_min, raw_max = np.percentile(raw_data_to_plot, [1, 99]); img_raw.set_clim(vmin=raw_min, vmax=raw_max if raw_max > raw_min else raw_min + 1)
        ...
        rfft_min, rfft_max = np.percentile(range_fft_db, [5, 99]); img_range_fft.set_clim(vmin=rfft_min, vmax=rfft_max if rfft_max > rfft_min else rfft_min + 1)
        ...
        s_min, s_max = np.percentile(spectrogram_db, [5, 99]); img_doppler_fft.set_clim(vmin=s_min, vmax=s_max if s_max > s_min else s_min + 1)
        ...
        axs[3].clear()
        axs[3].text(0.1, 0.9, 'Advanced Motion Scoring', fontsize=16, weight='bold')
        ...
        axs[3].text(0.1, 0.5, f"Max Motion Score: {motion_level:,.0f}", fontsize=12)
